...
@section("content")
    <div class="w-full flex justify-center items-center" x-data="editRol(@json($permissions), {{$role->id}})">
        ...
        <x-modal-confirm-component :id="'showConfirmRemovePermission'" message="Desae remover el permiso">
            <x-slot name="confirmButton">
                <button @click="confirmRemovePermission()" class="bg-brand-alert p-2 text-white text-sm rounded-md w-14 cursor-pointer">
                    Si
                </button>
            </x-slot>
        ...
        <div class="bg-white p-5 rounded-md mt-15 w-4/5">
            <div class="">
                <h3 class="font-bold text-gray-700">Edicion de rol</h3>
            </div>
            <div class="mt-10">
                <form action="{{route("roleManager.update", $role->id)}}" method="post" x-ref="formCreateRol">
                    @csrf
                    @method("PUT")
                    <div>
                    ...
                    <div>
                        ...
                        <input name="name" value="{{$role->name}}" class="w-full text-sm border p-1 border-gray-400 rounded-sm outline-0 mt-1" type="text" required>
                    </div>
                    <div class="mt-3">
                        <div class="relative w-full">
                            ...
                            <template x-if="showListModules">
                                <div @scroll.passive="($el.scrollHeight - $el.scrollTop <= $el.clientHeight) && loadMoreModules()" id="periods-container" class="overflow-y-auto h-20 absolute top-full left-0 w-full border border-gray-400 bg-white rounded-sm z-50 group-focus-within:pointer-events-auto">
                            ...
                            <template x-if="!isLoadingListPermissions">
                                <div class="relative flex flex-col my-6 bg-white shadow-sm border border-slate-200 rounded-lg w-full">
                                    <div class="p-4">
                                        ...
                                        <div @scroll.passive="($el.scrollHeight - $el.scrollTop <= $el.clientHeight + 1) && loadMorePermissions()" class="divide-y divide-slate-200 h-52 overflow-y-auto p-2">
                    ...
                    <div class="mt-3">
                        ...
                        <textarea name="description" placeholder="Descripcion" class="mt-1 text-sm border border-gray-400 rounded-md p-1 outline-0 w-full">{{$role->description}}</textarea>
                    </div>
                    <div class="mt-5 flex justify-end w-full">
                        <button type="submit" class="cursor-pointer p-2 text-white bg-brand-primary rounded-md text-sm">
                            Editar
                        </button>
                    </div>
                </form>
