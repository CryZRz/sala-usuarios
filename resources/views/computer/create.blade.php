...
@section("content")
    <div class="mt-10 mx-8" x-data="createComputer()">

        <x-modal-component :id="'createPortModal'" title="Crear Puerto">
            <x-slot name="body">
                <form action="">
                    <div>
        ...
        <x-modal-component :id="'editPortModal'" title="Editar Puerto">
            <x-slot name="body">
                <form action="">
                    <div>
                        <label class="block text-sm text-gray-800 font-bold" for="input-port-type">Puerto: </label>
        ...
        <x-modal-component :id="'addProgram'" title="Agregar programa">
            <x-slot name="body">
                ...
                <div id="dropdownSearch" class="z-10 bg-white rounded-lg shadow-sm w-full">
                    ...
                    <ul @scroll.passive="($el.scrollHeight - $el.scrollTop <= $el.clientHeight +2) && loadMore()" class="h-48 px-3 pb-3 overflow-y-auto text-sm text-gray-700">
                        <template x-if="!isLoadingPrograms">
                            <template x-for="programAdd in programsToAdd">
                                <li>
                                    <div class="flex items-center p-2 rounded-sm hover:bg-gray-100">
                                        <input :checked="verifyProgramSelect(programAdd)" @change="addProgramSelect($event, programAdd)" id="checkbox-item-11" type="checkbox" value="" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 focus:ring-2">
        ...
        <div class="w-full p-6 bg-white rounded-md">
            <div class="w-full  mt-0">
                <div>
                    <form class="flex justify-evenly gap-2" action="" method="POST" id="mainForm">
                        @csrf
                        <div class="w-full">
                            <label class="block text-sm text-gray-700 font-semibold" for="computerNumber">Num. Equipo</label>
                            <input x-model="computerNumber" id="computerNumber" name="computerNumber" type="text" class="text-sm outline-0 w-full border border-gray-400 rounded-md p-1">
                        ...
                        <div class="w-full">
                            ...
                            <input x-model="cpu" id="cpu" name="cpu" type="text" class="text-sm outline-0 w-full border border-gray-400 rounded-md p-1">
                            @error("cpu")
                            <p class="text-red-400 text-sm mt-2">
                                {{$message}}
                            </p>
                        ...
                        <div class="w-full">
                            ...
                            <input x-model="ram" id="ram" type="number" name="ram" class="text-sm outline-0 w-full border border-gray-400 rounded-md p-1">
                            @error("ram")
                            <p class="text-red-400 text-sm mt-2">
                                {{$message}}
                            </p>
                        ...
                        <div class="relative flex flex-col my-6 bg-white shadow-sm border border-slate-200 rounded-lg w-full">
                            <div class="p-4">
                                <div class="mb-4 flex items-center justify-between">
                                    ...
                                    <button @click="createPortModal = true" class="bg-brand-secondary rounded-md p-2 text-sm text-white cursor-pointer">
                                        Añadir
                                    </button>
                                </div>
                                <div class="w-full">
